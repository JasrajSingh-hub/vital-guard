...
const PatientCard: React.FC<PatientCardProps> = ({ patient, onClick }) => {
  ...
  const getStatusBadge = (risk: RiskLevel) => {
    switch (risk) {
      case 'CRITICAL':
        return <span className="px-2 py-1 rounded-full text-xs font-bold bg-red-600 text-white">CRITICAL</span>;
      case 'ATTENTION':
        return <span className="px-2 py-1 rounded-full text-xs font-bold bg-amber-500 text-white">ATTENTION</span>;
      case 'STABLE':
        return <span className="px-2 py-1 rounded-full text-xs font-bold bg-emerald-600 text-white">STABLE</span>;
    }
  };
    ...
    >
      <div className="flex justify-between items-start mb-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">UID: {patient.patientUid || patient.id} | Room: {patient.room}</p>
          {isContextOnly && (
            <span className="inline-block mt-1 px-2 py-0.5 bg-slate-100 text-slate-700 text-xs font-semibold rounded">
              Care Coordination
            </span>
          )}
        </div>
      ...
      {isContextOnly ? (
        <div className="space-y-2">
          ...
          {patient.context?.diagnosis && (
            <div className="text-xs text-slate-600 mt-2 line-clamp-2">{patient.context.diagnosis}</div>
          )}
        </div>
      ) : latest ? (
        <>
          <div className="grid grid-cols-2 gap-4 mb-3">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm font-medium">
                {latest.heartRate} <span className="text-slate-500 text-xs">bpm</span>
              </span>
            </div>
            <div className="flex items-center space-x-2">
              <Activity className="w-4 h-4 text-blue-500" />
              <span className="text-sm font-medium">
                {latest.systolicBp}/{latest.diastolicBp} <span className="text-slate-500 text-xs">mmHg</span>
              </span>
            </div>
            <div className="flex items-center space-x-2">
              <Wind className="w-4 h-4 text-cyan-500" />
              <span className="text-sm font-medium">
                {latest.spO2}% <span className="text-slate-500 text-xs">SpO2</span>
              </span>
            </div>
            <div className="flex items-center space-x-2">
          ...
          {patient.aiAnalysis && (
            <div className="mt-3 p-2 bg-white/70 rounded text-xs text-slate-700 flex items-start">
